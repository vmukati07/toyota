...
use Magento\Catalog\Model\Product as ProductAttribute;
...
class Product extends \Magento\CatalogImportExport\Model\Import\Product
{
    ...
    protected ProductAttribute $product;
     ...
     * @return array
     */
    protected function getExistingAttributesData($bunch)
    {
        ...
        $productCollection = $productCollection->addAttributeToSelect('hazmat_flag', true)
        ->addAttributeToSelect('do_not_ship', true)
        ->addAttributeToSelect('price', true)
        ->addAttributeToSelect('cost', true)
        ->addAttributeToSelect('status', true)
        ->addAttributeToSelect('saleable', true)
        ->addAttributeToSelect('tier_price_set', true);

        $isEnabled = $this->helper->getConfig('website_products_mapping/general/active');

        //assign existing website ids if `assign all products to store` config value set as no
        if (!$isEnabled) {
            $productCollection->getSelect()->joinLeft(
                ['websites' => 'catalog_product_website'],
                "websites.product_id = e.entity_id",
                [
                    'website_ids' => "GROUP_CONCAT(websites.website_id SEPARATOR ',')"
                ]
            )->group('websites.product_id');
        }
        ...
        if (!empty($attributesData)) {
            array_walk(
                $attributesData,
                function (&$val, $key) use (&$finalData, &$attribute, &$isEnabled) {
                    $sku = $val['sku'];
                    $finalData[$sku] = [
                        'hazmat_flag' => $val['hazmat_flag'],
                        'do_not_ship' => $val['do_not_ship'],
                        'price' => $val['price'],
                        'cost' => $val['cost'],
                        'status' => $val['status'],
                        'saleable' => $val['saleable'],
                        'tier_price_set' => $val['tier_price_set'],
                    ];
                    if (!$isEnabled) {
                        $finalData[$sku]['website_ids'] = $val['website_ids'];
                    }
                }
            );
        }
    ...
    protected function _saveProducts()
    {
        ...
        while ($bunch = $this->_dataSourceModel->getNextBunch()) {
            ...
            //fetch existing value for attributes to match with csv data
            $existingAttributesData = $this->getExistingAttributesData($bunch);
            ...
            foreach ($bunch as $rowNum => $rowData) {
                ...
                $existingHazmatFlag = isset($existingAttributesData[$rowSku]['hazmat_flag']) ?
                    $existingAttributesData[$rowSku]['hazmat_flag'] : '';
                $existingDoNoShip = isset($existingAttributesData[$rowSku]['do_not_ship']) ?
                    $existingAttributesData[$rowSku]['do_not_ship'] : '';
                
                // assign existing value if attribute value is missing in csv
                $hazmatFlag = isset($rowData['hazmat_flag']) && !empty($rowData['hazmat_flag']) ?
                    $rowData['hazmat_flag'] : $existingHazmatFlag;
                $doNotShip = isset($rowData['do_not_ship']) && !empty($rowData['do_not_ship']) ?
                    $rowData['do_not_ship'] : $existingDoNoShip;
                
                //if hazmat or do not ship is set to yes
                ...
                //Changes for product status
                $existingPrice = isset($existingAttributesData[$rowSku]['price']) ?
                    $existingAttributesData[$rowSku]['price'] : 0;
                $existingStatus = isset($existingAttributesData[$rowSku]['status']) ?
                    $existingAttributesData[$rowSku]['status'] : 1;
                
                // assign existing value if attribute value is missing in csv
                $price = isset($rowData['price']) ? $rowData['price'] : $existingPrice;
                $productOnline = isset($rowData['product_online']) ?
                    $rowData['product_online'] : $existingStatus;
                $thresholdPrice = $this->helper->getThresholdPrice();

                //check if price is below threshold Price
                $isBelowThreshold = isset($thresholdPrice) && $price <= $thresholdPrice;

                // If price is below threshold, always disable
                if ($isBelowThreshold || $productOnline==2) {
                    $productOnline = 2;
                } else {
                    $productOnline = 1;
                }
                ...
                if($aapProductsStatus) {
                    $tierPriceSetData = "";
                    // assign existing value if attribute value is missing in csv
                    if (isset($rowData['tier_price_set']) && $rowData['tier_price_set'] != $this->getEmptyAttributeValueConstant()) {
                        $tierPriceSetData = $rowData['tier_price_set'];
                    } else {
                        $tierPriceSet = isset($existingAttributesData[$rowSku]['tier_price_set']) ? $existingAttributesData[$rowSku]['tier_price_set'] : '';
                        
                        //Fetch option value by attribute option id
                        if (!empty($tierPriceSet)) {
                            $attribute = $this->product->getResource()->getAttribute('tier_price_set');
                            if ($attribute->usesSource()) {
                                $tierPriceSetData = $attribute->getSource()->getOptionText($tierPriceSet);
                            }
                        }
                    }

                    //Disable product if tier_price_set is AAP
                    if ($tierPriceSetData == 'AAP') {
                        $productOnline = 2;
                    }
                }
                
                $rowData['status'] = $productOnline;
                ...
                if (isset($rowData['model_year_code']) && $rowData['model_year_code'] != $this->getEmptyAttributeValueConstant()) {
                ...
                foreach ($rowImages as $column => $columnImages) {
                    foreach ($columnImages as $columnImageKey => $columnImage) {
                        if (!array_key_exists($rowSku, $productMediaData) ||
                            (array_key_exists($rowSku, $productMediaData) && !in_array($columnImage, $productMediaData[$rowSku]))
                        ) {
            ...
            $this->_eventManager->dispatch(
                ...
                ['adapter' => $this, 'bunch' => $bunch, 'website_data' => $existingAttributesData]
            );
        }
    ...
    private function customColValues($rowData)
    {
        ...
        if ($isEnabled &&
            (!isset($rowData['website']) || (isset($rowData['website']) && $rowData['website'] == $this->getEmptyAttributeValueConstant()))) {
        ...
        } elseif (isset($rowData['website']) && $rowData['website'] != $this->getEmptyAttributeValueConstant()) {
            $rowData['product_websites'] = $rowData['website'];
            $rowData['product_website_ids'] = implode(",", $this->getWebsiteIdsByCode($rowData['website']));
        }

        $sku[]['sku'] = $rowData['sku'];
        $existingAttributesData = $this->getExistingAttributesData($sku);
        $stockAAPConfig = $this->helper->getProductStockStatus();
        if($stockAAPConfig) {
            $tierPriceSetData = "";
            if(isset($rowData['saleable']) && !empty($rowData['saleable'])) {
                $saleableData = $rowData['saleable'];
            } else {
                $saleableData = isset($existingAttributesData[$rowData['sku']]['saleable']) ? $existingAttributesData[$rowData['sku']]['saleable'] : 'N';
            }

            if(isset($rowData['tier_price_set']) && !empty($rowData['tier_price_set'])) {
                $tierPriceSetData = $rowData['tier_price_set'];
            } else {
                $tierPriceSet = isset($existingAttributesData[$rowData['sku']]['tier_price_set']) ? $existingAttributesData[$rowData['sku']]['tier_price_set'] : '';
            ...
            if ($saleableData == 'Y' && $tierPriceSetData != 'AAP') {
            ...
            if (isset($rowData['saleable']) && !empty($rowData['saleable'])) {
                ...
                if ($saleable == 'Y') {
                    $rowData['is_in_stock'] = true;
                    $rowData['manage_stock'] = 0;
                } elseif ($saleable == $this->getEmptyAttributeValueConstant() || $saleable == 'N') {
                    $rowData['is_in_stock'] = false;
                    $rowData['manage_stock'] = 1;
                }
            }
        }
        //check product price,cost, tier_price_set change to recalculate dealer price
        $rowData['update_dealer_price'] = 0;
        if(isset($rowData['tier_price_set']) && !empty($rowData['tier_price_set'])) {
            $existingProductTierPriceSet = isset($existingAttributesData[$rowData['sku']]['tier_price_set']) ? $existingAttributesData[$rowData['sku']]['tier_price_set'] : '';
            $existingProductTierPriceSetLabel = '';          
            //Fetch option value by attribute option id
            if (!empty($existingProductTierPriceSet)) {
                $attribute = $this->product->getResource()->getAttribute('tier_price_set');
                if ($attribute->usesSource()) {
                    $existingProductTierPriceSetLabel = $attribute->getSource()->getOptionText($existingProductTierPriceSet);
                }
            }
        }

        if(
            (isset($rowData['price']) && !empty($rowData['price']) && isset($existingAttributesData[$rowData['sku']]['price']) && $existingAttributesData[$rowData['sku']]['price'] != $rowData['price'])
            || (isset($rowData['cost']) && !empty($rowData['cost']) && isset($existingAttributesData[$rowData['sku']]['cost']) && $existingAttributesData[$rowData['sku']]['cost'] != $rowData['cost'])
            || (isset($rowData['tier_price_set']) && !empty($rowData['tier_price_set']) && isset($existingProductTierPriceSetLabel) && $existingProductTierPriceSetLabel != $rowData['tier_price_set'])
        ) {
            $rowData['update_dealer_price'] = 1;
        }
        //end of check product price,cost, tier_price_set change to recalculate dealer price

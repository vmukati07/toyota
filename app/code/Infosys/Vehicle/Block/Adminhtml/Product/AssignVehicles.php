...
namespace Infosys\Vehicle\Block\Adminhtml\Product;

use Infosys\Vehicle\Block\Adminhtml\Product\Tab\VehicleGrid;

class AssignVehicles extends \Magento\Backend\Block\Template
{
    /**
     * Block template
     *
    ...
    public function getProductsJson()
    {
        ...
        $vehicleproductMappingFactory->addFieldToSelect(['vehicle_id']);
        $vehicleproductMappingFactory->addFieldToFilter('product_id', ['eq' => $entity_id]);
        $result = [];
        if (!empty($vehicleproductMappingFactory->getData())) {
            foreach ($vehicleproductMappingFactory->getData() as $vehicleProducts) {
                $result[$vehicleProducts['vehicle_id']] = '';
            }

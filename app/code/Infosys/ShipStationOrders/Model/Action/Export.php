...
namespace Infosys\ShipStationOrders\Model\Action;
...
use Auctane\Api\Model\Action\Export as ShipStationExport;

/**
 * Class Export
 * @package Auctane\Api\Model\Action
 */
class Export extends ShipStationExport
{
    /**
    ...
    private ShipStationOrdersLogger $logger;
    ...
    public function process(HttpRequest $request, array $storeIds): string
    {
        ...
        try {
            if ($from && $to) {
                ...
                //Restrict in store pickup orders and DF, manual shipped order items to sync in Ship station under Await shipping
                $orders->getSelect()->join(['item' => 'sales_order_item'], "main_table.entity_id = item.order_id", [''])
                    ->where('item.updated_at >= (?)', $from)
                    ->where('item.updated_at <= (?)', $to)
                    ->where('main_table.shipping_description != (?)', null)
                    ->where('main_table.is_store_pickup = (?)', 0);
                if (!empty($storeIds)) {
                    $orders->getSelect()->where('main_table.store_id in (?)', $storeIds);
                }
                $orders->getSelect()->distinct(true)
                ...
                $this->logger->info("Orders are " . json_encode($orders->getColumnValues('entity_id')) . " from " . $from . " to " . $to);
            ...
            $this->logger->error("Error while importing orders to shipstation" . $e->getMessage());
        }
    ...
    private function _orderItem(Order $order): self
    {
        ...
        foreach ($order->getItems() as $orderItem) {
            ...
            // Exclude DF shipped items
            if (!empty($orderItem->getDirectFulfillmentStatus()) &&
                !str_contains(strtoupper($orderItem->getDirectFulfillmentStatus()), 'REJECTED') && 
                !str_contains(strtoupper($orderItem->getDirectFulfillmentStatus()), 'CANCELLED')
            ) {
                continue;
            }

            //Exclude already manually shipped items
            $remainingQty = $orderItem->getQtyOrdered();
            if (!empty($orderItem->getQtyShipped())) {
                if ($orderItem->getQtyShipped() == $orderItem->getQtyOrdered()) {
                    continue;
                } else {
                    $remainingQty = $orderItem->getQtyOrdered() - $orderItem->getQtyShipped();
                }
            }
            ...
            $this->addXmlElement("Quantity", "<![CDATA[" . (int)$remainingQty . "]]>");
